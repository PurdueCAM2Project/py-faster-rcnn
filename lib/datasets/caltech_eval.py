...
def parse_rec(annopath,annotations):
    """ Parse a PASCAL VOC xml file """
    idx = annopath.split("_")
    anno = annotations[idx[0]][idx[1]]['frames'][idx[2]]
    objects = []
    for obj in anno:
        obj_struct = {}
...
def caltech_eval(detpath,
    ...
    with open(imagesetfile, 'r') as f:
        lines = f.readlines()
    imagenames = [x.strip().replace(" ","_") for x in lines]
    ...
    if not os.path.isfile(cachefile):
        ...
        for i, imagename in enumerate(imagenames):
            try:
                recs[imagename] = parse_rec(imagename,annotations)
            except:
                print("grep_me imgname: {}".format(imagename))
                recs[imagename] = parse_rec(imagename,annotations)
