...
class sun(imdb):
    ...
    def _get_default_path(self):
        """
        Return the default path where SUN is expected to be installed.
        """
    ...
    def _load_sun_annotation(self, index):
        """
        Load image and bounding boxes info from XML file in the SUN
        format.
        """
        ...
        # "Seg" area for sun is just the box area
    ...
    def _get_sun_results_file_template(self):
        # SUNdevkit/results/SUN2007/Main/<comp_id>_det_test_aeroplane.txt
    ...
    def _write_sun_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            ...
            filename = self._get_sun_results_file_template().format(cls)
            print(filename)
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        aps = []
        # The SUN metric changed in 2010
        use_07_metric = True if int(self._year) < 2010 else False
        print "File reading from:{}".format(imagesetfile)
        print 'SUN07 metric? ' + ('Yes' if use_07_metric else 'No')
    ...
    def _do_matlab_eval(self, output_dir='output'):
        ...
        path = os.path.join(cfg.ROOT_DIR, 'lib', 'datasets',
                            'SUNdevkit-matlab-wrapper')
        cmd = 'cd {} && '.format(path)
        cmd += '{:s} -nodisplay -nodesktop '.format(cfg.MATLAB)
        cmd += '-r "dbstop if error; '
        cmd += 'sun_eval(\'{:s}\',\'{:s}\',\'{:s}\',\'{:s}\'); quit;"' \
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_sun_results_file(all_boxes)
