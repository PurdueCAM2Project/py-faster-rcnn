...
import subprocess
import uuid
from caltech_eval import caltech_eval
from fast_rcnn.config import cfg

class caltech(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'caltech_' + year + '_' + image_set)
        self._year = year
        ...
        self._data_path = os.path.join(self._devkit_path, 'CAL' + self._year)

        # load files
        annos = os.path.join(self._data_path,"annotations.json")
        with open(annos,'r') as f:
            self.annotations = json.load(f)
        meta_a = os.path.join(self._data_path,"meta_data_l1.p")
        with open(meta_a,'r') as f:
            self.meta_data_l1 = cPickle.load(f)
        meta_b = os.path.join(self._data_path,"meta_data_l2.p")
        with open(meta_b,'r') as f:
            self.meta_data_l2 = cPickle.load(f)
        ...
        assert os.path.exists(self._devkit_path), \
                'CAL path does not exist: {}'.format(self._devkit_path)
        assert os.path.exists(self._data_path), \
                'Path does not exist: {}'.format(self._data_path)

    def index_to_fields(self,i):
        # Usage: set##,V### = self.index_to_fields(i)
        
        #m = re.match(r".*set(?P<set>[0-9]{2}).*V(?P<vn>[0-9]{3})",rn)
        # if m is None: sys.exit(1)
        # mdic = m.groupdict()
        set_num = ""
        for sets in sorted(self.meta_data_l1.keys()):
            print(sets)
            if self.meta_data_l1[sets] <= i: set_num = sets
            else: break
        assert(set_num != ""),"Error: set_num is not assigned"

        v_num = ""
        for vs in sorted(self.meta_data_l2[set_num].keys()):
            if self.meta_data_l2[set_num][vs] <= i: v_num = vs
            else: break
        assert(v_num != ""),"Error: v_num is not assigned"

        # print(i)
        # print(self.meta_data_l2[set_num][v_num])
        rem = int(i) - int(self.meta_data_l2[set_num][v_num])
        
        fn = set_num + "_" + v_num + "_" + str(rem)
    ...
    def _load_image_set_index(self):
        ...
        # self._devkit_path + /Caltech/ImageSets/Main/all.txt
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'CALdevkit' + self._year)
    ...
    def _load_caltech_annotation(self, index):
        ...
        indicies = index.split("_")
        objs = self.annotations[indicies[0]][indicies[1]]['frames'][indicies[2]]
        ...
        if self.config['only_people']:
            only_people = [
                obj for obj in objs \
                if "person" in obj['lbl'].lower().strip()
            ]
            objs = only_people
        ...
        # "Seg" area for cal is just the box area
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} CAL results file'.format(cls)
            ...
            with open(filename, 'wt') as f:
                for im_ind, index in enumerate(self.image_index):
                    ...
                    if dets == []:
                        print("NOTHING")
                        continue
                    # i am not sure if CAL is "+1" or not so I will just be consistent with "+0"
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            filename = self._get_voc_results_file_template().format(cls)
            rec, prec, ap, ovthresh = caltech_eval(
                filename, self.annotations, imagesetfile, cls, cachedir, ovthresh=0.5,
                use_07_metric=use_07_metric)
            aps += [ap]
        aps = np.array(aps)
        results_fd = open("./results_caltech.txt","w")
