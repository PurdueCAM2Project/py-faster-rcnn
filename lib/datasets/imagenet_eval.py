...
def parse_rec(filename):
    ...
    if(re.search(r"\ [0-9]+\.xml",filename)):
...
def voc_eval(detpath,
    ...
    print "Detfile BB code area {}".format(detfile)
    ...
    for idx in range(len(ovthresh)):
        ...
        #ap = voc_ap(rec, prec, use_07_metric)
        ap[idx] = voc_ap(rec[:,idx], prec[:,idx],False)
